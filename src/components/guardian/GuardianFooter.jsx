...
const SOCIAL_LINKS = {
  twitter: 'https://twitter.com/yanci_ng',
  facebook: 'https://facebook.com/yanci.ng',
            ...
            <div className="bg-white/5 p-6 rounded-sm border border-white/10 backdrop-blur-sm">
              ...
              {submitted ? (
                <div className="flex items-center gap-2 text-green-400 text-sm font-bold">
                  <FaCircleCheck className="w-4 h-4" /> An yi rajista!
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="flex gap-2">
                  ...
                  />
                  <button type="submit" className="bg-yanci-accent text-yanci-primary px-4 py-2 rounded-sm hover:bg-white transition-colors">
          ...
          <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-8">
            <div>
              ...
              <ul className="space-y-4 text-sm text-gray-400">
                {[['Najeriya', '/labarai'], ['Siyasa', '/siyasa'], ['Kasuwanci', '/kasuwanci'], ['Wasanni', '/wasanni'], ['Fasaha', '/fasaha'], ['Zabe', '/zabe']].map(([item, path]) => (
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm text-gray-400">
                {[['Sharhin Edita', '/raayi'], ['Bincike', '/raayi'], ['Tattaunawa', '/raayi']].map(([item, path]) => (
                  <li key={item}><a href={path} className="hover:text-white hover:translate-x-1 transition-all block">{item}</a></li>
                ))}
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm text-gray-400">
                {[['Kiɗa', '/aladu'], ['Fina-finai', '/bidiyo'], ['Abinci', '/aladu']].map(([item, path]) => (
                  <li key={item}><a href={path} className="hover:text-white hover:translate-x-1 transition-all block">{item}</a></li>
                ))}
              </ul>
            </div>
