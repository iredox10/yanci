...
const AdminHomepageStats = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const docs = await appwriteService.getHomepageStats();
        if (docs.length > 0) {
          setStats(docs.map(doc => ({ ...doc, id: doc.$id })));
          setUseAppwrite(true);
          return;
        }
      } catch {}
      const saved = localStorage.getItem(STORAGE_KEY);
      setStats(saved ? JSON.parse(saved) : initialStats);
    };
    load();
  }, []);
  ...
  const persist = async (data) => {
    if (useAppwrite) {
      try {
        if (data.$id) {
          await appwriteService.updateHomepageStat(data.$id, data);
        } else {
          await appwriteService.createHomepageStat(data);
        }
      } catch {}
    }
  ...
  const handleSave = async () => {
    setSaving(true);
    const data = { ...draft, order: draft.order ?? stats.length };
    let updated;
    if (draft.id && draft.$id) {
      updated = stats.map(s => s.$id === draft.$id ? { ...s, ...data } : s);
    } else {
      updated = [...stats, { ...data, id: Date.now() }];
    }
    setStats(updated);
  ...
  const handleDelete = async (item) => {
    if (!window.confirm('Share wannan statistic?')) return;
    if (useAppwrite && item.$id) {
      try { await appwriteService.deleteHomepageStat(item.$id); } catch {}
    }
    const updated = stats.filter(s => s.$id !== item.$id && s.id !== item.id);
    setStats(updated);
  ...
  const openEdit = (stat) => { setDraft(stat); setIsModalOpen(true); };
  const openNew = () => { setDraft(emptyStat); setIsModalOpen(true); };
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Gudanar da lambobin da ke shafin farko {useAppwrite ? '(Appwrite)' : '(Local)'}</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-8">
          {stats.map(stat => (
            <div key={stat.id} className="text-center">
              <div className="text-3xl md:text-4xl font-serif font-black text-white mb-1">{stat.value.toLocaleString()}{stat.suffix}</div>
      ...
      <div className="space-y-3">
        {stats.map((stat, index) => (
          <div key={stat.$id || stat.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="text-2xl font-black text-[#0f3036]">{stat.value.toLocaleString()}{stat.suffix}</div>
              <div><p className="font-bold text-gray-900">{stat.label}</p></div>
            </div>
            <div className="flex gap-1">
              ...
              <button onClick={() => handleDelete(stat)} className="p-1.5 text-red-600 hover:bg-red-50 rounded"><FaTrash className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={draft.label} onChange={e => setDraft({ ...draft, label: e.target.value })} placeholder="Masu karatu, Labarai, etc." className="w-full text-sm p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#c59d5f] outline-none" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Value (Number)</label>
                <input type="number" value={draft.value} onChange={e => setDraft({ ...draft, value: parseInt(e.target.value) || 0 })} className="w-full text-sm p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#c59d5f] outline-none" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase">Suffix (e.g. +, K+, %)</label>
                <input value={draft.suffix} onChange={e => setDraft({ ...draft, suffix: e.target.value })} placeholder="+, K+, %, etc." className="w-full text-sm p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#c59d5f] outline-none" />
              </div>
            </div>
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
              ...
              <button onClick={handleSave} disabled={saving || !draft.label} className="px-6 py-2 text-sm font-bold text-white bg-[#0f3036] hover:bg-[#1a454c] rounded-lg disabled:opacity-50 flex items-center gap-2">
