...
const initialHighlights = [
  ...
  },
  {
    id: 3,
    tag: 'Kirkire-kirkire',
    title: 'Matasa a Zaria sun ƙirƙira manhajar gano gonaki ta tauraron dan adam',
...
const AdminHighlights = () => {
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      if (draft.id) {
        setHighlights(prev => prev.map(h => h.id === draft.id ? draft : h));
      } else {
        setHighlights(prev => [...prev, { ...draft, id: Date.now() }]);
      }
  ...
  const handleDelete = (id) => {
    if (window.confirm('Share wannan highlight?')) {
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Gudanar da manyan labarai a shafin farko (Homepage Feature Cards)</p>
        </div>
        <button
          onClick={openNew}
      ...
      <div className="space-y-4">
        {highlights.map((item, index) => (
          <div
            key={item.id}
            className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex flex-col sm:flex-row gap-4"
          >
            {/* Preview */}
            <div
              className="w-full sm:w-48 h-32 rounded-lg flex flex-col items-center justify-center shrink-0 p-4 text-white"
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex gap-1 shrink-0">
                  <button onClick={() => handleMove(index, 'up')} disabled={index === 0} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg disabled:opacity-30">
                    <FaArrowUp className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleMove(index, 'down')} disabled={index === highlights.length - 1} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg disabled:opacity-30">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-lg w-full p-6 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="Kasuwanci, Sauti, etc."
              ...
              <div>
                ...
                <textarea
                  value={draft.copy} onChange={e => setDraft({ ...draft, copy: e.target.value })}
                  rows="3"
              ...
              <div>
                ...
                <select
                  value={draft.icon} onChange={e => setDraft({ ...draft, icon: e.target.value })}
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {COLOR_PRESETS.map((preset) => (
                    <button
                      key={preset.accent}
                      onClick={() => setDraft({ ...draft, accent: preset.accent, gradient: preset.gradient })}
                      className={`w-10 h-10 rounded-lg border-2 transition-all ${draft.accent === preset.accent ? 'border-gray-800 scale-110' : 'border-transparent'}`}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-bold text-gray-500 hover:bg-gray-100 rounded-lg">
                Soke
              </button>
              <button
                onClick={handleSave} disabled={saving || !draft.title || !draft.tag}
